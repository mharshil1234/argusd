...
function ClaimRow({ claim }: { claim: DashboardClaim }) {
  return (
    <article className="claim-row">
      <div className="claim-copy"><p>{claim.text}</p><code>{claim.sourceKey}</code></div>
      <div className="claim-meta">
        <span className={`status status-${claim.status}`}><span className="status-dot" aria-hidden="true" />{claim.status}</span>
  ...
  return (
    <main className="dashboard-shell">
      <header className="dashboard-header">
        ...
        <div className={`connection connection-${data.state}`} aria-live="polite"><span className="connection-dot" aria-hidden="true" /><div><strong>{connectionLabel}</strong><span>{data.generatedAt ? `Updated ${formatTimestamp(data.generatedAt)}` : "Starting poller"}</span></div></div>
      </header>
      ...
      <section className="claims-panel" aria-labelledby="claims-heading">
        <div className="panel-header"><div><p className="section-kicker">READ-ONLY SQLITE VIEW</p><h2 id="claims-heading">Tracked claims</h2></div><span className="poll-rate">POLLING · 1s</span></div>
        {loading ? <div className="state-card" role="status"><span className="spinner" />Reading claims…</div>
          : data.state !== "ready" ? <div className={`state-card state-${data.state}`} role="status"><strong>{data.state === "waiting" ? "Argusd is ready for data" : "Claims are temporarily unavailable"}</strong><p>{data.message}</p>{data.state === "waiting" && <code>python -c &quot;from server.db import connect, init_db; c=connect(); init_db(c)&quot;</code>}</div>
