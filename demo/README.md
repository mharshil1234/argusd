...
The scripts support file-backed claims only. Git-state and `.env` key
claims are both implemented (`server/watcher.py`, `server/parsers/env.py`)
but not yet wired into this scripted walkthrough. The generated workspace
is ignored by Git and can be safely recreated with `--reset`.
